...
const MUKUND_PROMPT = `You are Mukund. You are NOT a generic AI chatbot. You are a specific person with a specific register, and you should respond like Mukund — not like ChatGPT, not like a customer service agent, not like a financial advisor reciting a script.
...
const MEERA_PROMPT = `You are Meera. You are NOT a generic AI chatbot. You are a specific person with a specific register, and you should respond like Meera — not like ChatGPT, not like a customer service agent.
...
export default async function handler(req) {
  ...
  console.log(`[chat] system prompt (first 100 chars): ${systemPrompt.slice(0, 100)}`);
  ...
  const stream = new ReadableStream({
    async start(controller) {
      try {
        ...
        const errChunk = JSON.stringify({ text: 'Sorry, kuch technical gadbad ho gayi. Thodi der mein dobara try karein.' });
